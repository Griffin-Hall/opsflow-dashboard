...
import type { KpiMetric, OperationsPayload, OpsOrder, OrderStatus, RefreshCycle } from "@/lib/types";
...
function simulationLabel(cycle: RefreshCycle) {
  return `May 30, 2026 • ${cycle === "7:00 AM" ? "7:03 AM" : "12:03 PM"}`;
}
      ...
      <div className="p-3">
        ...
        >
          <div className="flex items-center gap-2 text-cyan-200">
            <ShieldCheck />
            {!collapsed && <span className="text-sm font-medium">Demo Safe</span>}
          </div>
          {!collapsed && (
...
function SettingsView({
  ...
  return (
    <section className="grid gap-4 lg:grid-cols-3">
      {[
        ["Data Source", sourceLabel, "Reads the configured workbook on the server."],
        ["Refresh Cycle", cycle, "The demo toggles between morning and midday snapshots."],
        ["Rows Loaded", formatNumber(rowCount), "Current source rows available to the dashboard."],
      ...
      <div className="rounded-3xl border border-cyan-300/20 bg-cyan-300/[0.06] p-5 text-cyan-50 lg:col-span-3">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-cyan-100/80">
          ...
          If no workbook is available, the dashboard uses portfolio-safe sample data.
        </p>
      </div>
    </section>
  );
...
export function DashboardShell({ initialData }: { initialData: OperationsPayload }) {
  ...
  const metrics = React.useMemo(() => computeMetrics(orders), [orders]);
  const extraMetrics = React.useMemo(
    () => metrics.filter((m) => m.label === "Total SOs" || m.label === "Pipeline Value"),
    [metrics]
  );
  ...
  async function refreshData() {
    ...
    try {
      ...
      const refreshedOrders = simulateRefresh(payload.orders, nextCycle);
      setOrders(refreshedOrders);
      setSourceLabel(payload.sourceLabel);
      setRowCount(payload.rowCount);
      setCycle(nextCycle);
      toast.success("Refresh complete", {
        description: `Loaded ${refreshedOrders.length} order lines from ${payload.sourceLabel}. 3 new orders loaded.`,
      });
    } catch {
      ...
      <div className="relative flex min-h-dvh">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
              ...
              <div className="flex items-center gap-3">
                <Sheet>
                  ...
                  <SheetContent className="border-white/10 bg-slate-950 p-0" side="left">
                    ...
                    <Sidebar activeTab={tab} collapsed={false} onTabChange={setTab} />
                  </SheetContent>
                </Sheet>
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Last Refreshed: {simulationLabel(cycle)} • {sourceLabel} • {rowCount} rows
                  </p>
                </div>
              </div>
          ...
          <div className="flex flex-col gap-4 px-4 py-4 sm:px-6">
            {/* Instant pulse-check: four headline KPIs */}
            {refreshing ? (
            ...
            >
              ...
              <TabsContent className="mt-4" value="ready">
                <div className="flex flex-col gap-3">
                  <div className="flex items-center gap-2 rounded-2xl border border-emerald-300/20 bg-emerald-300/[0.06] px-4 py-3 text-sm text-emerald-100">
                    ...
                    {formatNumber(readyOrders.length)} orders pass every check and can ship on the
                    next cycle. Generate the daily ops sheet to release them.
                  </div>
              ...
              <TabsContent className="mt-4" value="settings">
                <SettingsView cycle={cycle} rowCount={rowCount} sourceLabel={sourceLabel} />
              </TabsContent>
            </Tabs>
          </div>
        </div>
